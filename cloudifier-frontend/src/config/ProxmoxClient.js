...
export default class ProxmoxClient {
  ...
  async getClusterResources(type = "") {
    try {
      ...
      const response = await this.instance.get("/cluster/resources", {
        params,
      });
      const res = await response.data;
      return res.data;
  ...
  async getVMStatus(node, vmid) {
    try {
      ...
      console.error(
        `Error getting status for VM ${vmid} on node ${node}:`,
        error
      );
      throw error;
    }
  ...
  async cloneVM(node, vmid, newid) {
    try {
      ...
      console.error(
        `Error cloning VM ${vmid} on node ${node} to ${newid}:`,
        error
      );
      throw error;
    }
  ...
  async migrateVM(node, vmid, target) {
    try {
      ...
      console.error(
        `Error migrating VM ${vmid} from node ${node} to ${target}:`,
        error
      );
      throw error;
    }
  ...
  async backupVM(node, vmid) {
    try {
      ...
      console.error(
        `Error creating backup of VM ${vmid} on node ${node}:`,
        error
      );
      throw error;
    }
  ...
  async getContainersList(node) {
    try {
      ...
      console.error(
        `Error getting LXC container index for node ${node}:`,
        error
      );
      throw error;
    }
  ...
  async createContainer(node, vmid, ostemplate) {
    try {
      ...
      console.error(
        `Error creating or restoring container on node ${node}:`,
        error
      );
      throw error;
    }
  ...
  async getContainerStatus(node, vmid) {
    try {
      ...
      console.error(
        `Error fetching status for container ${vmid} on node ${node}:`,
        error
      );
      throw error;
    }
  ...
  async suspendContainer(node, vmid) {
    try {
      ...
      console.error(
        `Error suspending container ${vmid} on node ${node}:`,
        error
      );
      throw error;
    }
  ...
  async shutdownContainer(node, vmid) {
    try {
      ...
      console.error(
        `Error shutting down container ${vmid} on node ${node}:`,
        error
      );
      throw error;
    }
  ...
  async rebootContainer(node, vmid) {
    try {
      ...
      console.error(
        `Error rebooting container ${vmid} on node ${node}:`,
        error
      );
      throw error;
    }
  ...
  async cloneContainer(node, vmid, newid) {
    try {
      ...
      console.error(
        `Error cloning container ${vmid} on node ${node} to ${newid}:`,
        error
      );
      throw error;
    }
  ...
  async migrateContainer(node, vmid, target) {
    try {
      ...
      console.error(
        `Error migrating container ${vmid} from node ${node} to ${target}:`,
        error
      );
      throw error;
    }
  ...
  async getContainerTemplates(node, storage) {
    try {
      ...
      console.error(
        `Error fetching templates from storage ${storage} on node ${node}:`,
        error
      );
      throw error;
    }
  ...
  async getBackups(node, storage) {
    try {
      ...
      console.error(
        `Error fetching backups from storage ${storage} on node ${node}:`,
        error
      );
      throw error;
    }
